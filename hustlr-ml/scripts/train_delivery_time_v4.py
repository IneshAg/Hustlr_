"""Train delivery-time model v4 with CV-backed selection and no-regression guard.

Usage:
  python scripts/train_delivery_time_v4.py
"""
...
CONFIG_PATH = ROOT / "config" / "delivery_time_v4.json"
...
def build_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["prep_time_min"] = prep.clip(lower=0, upper=240)
            ...
            (
                ...
                Pipeline(
                    [
                        ...
                        ("ohe", OneHotEncoder(handle_unknown="ignore", min_frequency=15)),
                    ]
                ),
                cat_cols,
            ),
...
def cv_mae(pipe: Pipeline, x: pd.DataFrame, y: pd.Series, seed: int, folds: int) -> tuple[float, float]:
    ...
    for tr_idx, va_idx in kf.split(x):
        ...
        p.fit(x_tr, y_tr)
        pred = p.predict(x_va)
        fold_scores.append(float(mean_absolute_error(y_va, pred)))
    return float(np.mean(fold_scores)), float(np.std(fold_scores))


def build_estimators(seed: int):
    return {
...
def main() -> int:
    ...
    for name, model in estimators.items():
        ...
        cv_mean, cv_std = cv_mae(pipe, x_train, y_train, seed=seed, folds=int(cfg["cv_folds"]))
    ...
    best_cv_name = min(all_results.keys(), key=lambda k: all_results[k]["cv_mae_mean"])
    best_cv_mae = float(all_results[best_cv_name]["cv_mae_mean"])
    best_holdout_name = min(all_results.keys(), key=lambda k: all_results[k]["holdout_mae"])

    if all_results[best_cv_name]["holdout_mae"] <= all_results[best_holdout_name]["holdout_mae"] + 0.15:
        best_name = best_cv_name
    else:
        best_name = best_holdout_name

    best_mae = float(all_results[best_name]["holdout_mae"])

    accepted = baseline_mae is None or best_mae <= baseline_mae
    if accepted:
        model_path = output_dir / f"delivery_time_best_v4_{best_name}.joblib"
        joblib.dump(fitted_models[best_name], model_path, compress=3)
        source = "new_v4"
    else:
    ...
    report = {
        ...
        "results": all_results,
        "best_cv_candidate": best_cv_name,
    ...
    report_path = output_dir / "delivery_time_improvement_report_v4.json"
